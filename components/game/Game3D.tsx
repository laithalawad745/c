// =====================================
// 📁 components/game/Game3D.tsx - Fixed Realistic Characters with Proximity System
// =====================================
'use client';
...
interface Game3DProps {
  ...
  players: any[];
  isLeader: boolean;
  onNearbyPlayerChange?: (nearbyPlayers: string[]) => void;
}
...
export default function Game3D({ 
  ...
  // المسافة المطلوبة للتفاوض (بالوحدات ثلاثية الأبعاد)
  const NEGOTIATION_DISTANCE = 3.0; // 3 وحدات
  ...
  // تحديث نظام فحص القرب
  const checkNearbyPlayers = () => {
    if (!gameRefs.current.otherPlayers) return;
    ...
    // فحص القرب من كل لاعب آخر
    gameRefs.current.otherPlayers.forEach((otherPlayerData, playerId) => {
      const distance = playerPos.distanceTo(otherPlayerData.group.position);
      
      if (distance <= NEGOTIATION_DISTANCE) {
        nearbyIds.push(playerId);
      }
    });
    
    // تحديث القائمة المحلية
    const newNearbySet = new Set(nearbyIds);
    
    // التحقق من التغييرات
    const hasChanged = nearbyIds.length !== nearbyPlayers.size ||
      nearbyIds.some(id => !nearbyPlayers.has(id));
    
    if (hasChanged) {
      setNearbyPlayers(newNearbySet);
      
      // إرسال التحديث للسيرفر
      SocketClient.emit('nearby-players-update', nearbyIds);
      
      // إرسال التحديث للكومبوننت الأب
      if (onNearbyPlayerChange) {
        onNearbyPlayerChange(nearbyIds);
      }
      
      console.log('Nearby players updated:', nearbyIds);
    }
  };

  // إضافة مؤشر بصري للاعبين القريبين
  const updatePlayerProximityIndicators = () => {
    if (!gameRefs.current.otherPlayers) return;
    
    gameRefs.current.otherPlayers.forEach((otherPlayerData, playerId) => {
      const isNearby = nearbyPlayers.has(playerId);
      
      // تحديث اللون أو الإضاءة للاعبين القريبين
      if (otherPlayerData.group) {
        const indicator = otherPlayerData.group.getObjectByName('proximityIndicator');
        
        if (isNearby && !indicator) {
          // إضافة مؤشر القرب
          ...
          ring.name = 'proximityIndicator';
          ring.rotation.x = -Math.PI / 2;
          ring.position.y = 0.01;
          otherPlayerData.group.add(ring);
          
          // إضافة تأثير نبض
          const pulseAnimation = () => {
            if (ring.parent) {
              ring.scale.x = 1 + Math.sin(Date.now() * 0.003) * 0.1;
              ring.scale.z = 1 + Math.sin(Date.now() * 0.003) * 0.1;
        ...
        } else if (!isNearby && indicator) {
          // إزالة مؤشر القرب
          otherPlayerData.group.remove(indicator);
        }
      }
    });
  };
  ...
  const animate = () => {
    ...
    // فحص اللاعبين القريبين كل frame (للقائد فقط)
    if (isLeader) {
      checkNearbyPlayers();
      updatePlayerProximityIndicators();
    }
    
    // Check nearby players for voice chat
    checkNearbyPlayersForVoice();
  ...
  const checkNearbyPlayersForVoice = () => {
  ...
  // عرض مؤشر المسافة في الواجهة
  const renderProximityHUD = () => {
    if (!isLeader) return null;
    
    return (
      <div className="absolute top-24 left-4 pointer-events-none">
        <div className="bg-black/60 backdrop-blur text-white p-3 rounded-xl">
          <p className="text-sm font-bold mb-2">📍 اللاعبون القريبون:</p>
          {players.filter(p => p.id !== SocketClient.id && p.isAlive).map(player => {
            const isNearby = nearbyPlayers.has(player.id);
            return (
              <div key={player.id} className="flex items-center gap-2 text-xs">
                <span className={`w-2 h-2 rounded-full ${isNearby ? 'bg-green-400' : 'bg-red-400'}`} />
                <span>{player.name}</span>
                {isNearby && <span className="text-green-400">✓ قريب</span>}
              </div>
            );
          })}
        </div>
          ...
          <div className="bg-black/70 backdrop-blur text-white px-6 py-4 rounded-xl text-center">
            ...
            {isLeader && (
              <div className="mt-3 p-2 bg-yellow-500/20 rounded-lg border border-yellow-400/30">
                <p className="text-yellow-400 font-bold">👑 أنت القائد</p>
                <p className="text-xs">اقترب من اللاعبين الآخرين للتفاوض معهم</p>
              </div>
            )}
          </div>
        </div>
