...
export default function GamePage() {
  ...
  // دالة لتحديث اللاعبين القريبين
  const handleNearbyPlayersUpdate = (playerIds: string[]) => {
    setNearbyPlayerIds(playerIds);
    console.log('Nearby players updated in parent:', playerIds);
  };
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      {currentPlayer && currentPlayer.part && (
        <>
          {/* 3D Game Component مع تمرير دالة القرب */}
          <Game3D
            roomId={roomId}
            ...
            onNearbyPlayerChange={handleNearbyPlayersUpdate}
          />
          
          {/* Negotiation System مع تمرير اللاعبين القريبين */}
          <NegotiationSystem
            roomId={roomId}
            playerName={currentPlayer.name}
            playerPart={currentPlayer.part}
          ...
          <ProximityVoiceChat 
            roomId={roomId}
            players={gameState.players}
            nearbyPlayer={nearbyPlayerIds.length > 0 ? 
              gameState.players.find(p => nearbyPlayerIds.includes(p.id)) : null}
          />
        </>
      )}

      {/* Game Info Panel مع عرض معلومات القرب */}
      <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-lg rounded-lg p-4 text-white z-30">
        ...
        <p>الجولة: {gameState?.roundNumber || 0}</p>
        <p>المرحلة: {getPhaseText(gameState?.currentPhase || 'waiting')}</p>
        <p>اللاعبون الأحياء: {gameState?.players.filter(p => p.isAlive).length || 0}</p>
        {currentPlayer?.part === 'chassis' && (
          <p className="text-yellow-400">
            لاعبون قريبون: {nearbyPlayerIds.length}
          </p>
        )}
      </div>
    </div>
