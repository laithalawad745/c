...
app.prepare().then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create-room', ({ roomId, playerName }) => {
      ...
      const room = {
        ...
        players: [{
          ...
          rotation: { y: 0 },
          nearbyPlayers: [] // إضافة قائمة اللاعبين القريبين
        }],
        ...
        roundNumber: 0,
        winners: [],
        negotiationDistance: 3.0 // المسافة المطلوبة للتفاوض
      };
    ...
    socket.on('player-move', ({ roomId, position, rotation, isRunning, isJumping }) => {
      const room = rooms.get(roomId);
      if (!room) return;
    ...
    // تحديث اللاعبين القريبين - للقائد فقط
    socket.on('nearby-players-update', (nearbyPlayerIds) => {
      const roomId = socket.data.roomId;
      const room = rooms.get(roomId);
      if (!room) return;

      const player = room.players.find(p => p.id === socket.id);
      if (!player || player.part !== 'chassis') return; // فقط القائد

      // تحديث قائمة اللاعبين القريبين
      player.nearbyPlayers = nearbyPlayerIds;

      // إرسال التحديث لجميع اللاعبين في الغرفة
      io.to(roomId).emit('proximity-update', {
        leaderId: socket.id,
        nearbyPlayers: nearbyPlayerIds
      });

      console.log(`👥 Leader ${player.name} has ${nearbyPlayerIds.length} nearby players`);
    });
    ...
    socket.on('start-game', (roomId) => {
      ...
      room.players.forEach((player, index) => {
        ...
        player.position = {
          x: Math.sin(angle) * 5,
          y: 1.6, // تم تصحيح الارتفاع
          z: Math.cos(angle) * 5
        };
        player.rotation = { y: -angle };
        player.nearbyPlayers = []; // إعادة تعيين القائمة
      });
    ...
    socket.on('negotiate', ({ roomId, targetPlayerId }) => {
      ...
      // فحص القرب باستخدام قائمة اللاعبين القريبين
      if (!requestingPlayer.nearbyPlayers.includes(targetPlayerId)) {
        socket.emit('error', 'اللاعب بعيد جداً - اقترب أكثر');
        return;
      }

      // التحقق من المسافة الفعلية كضمان إضافي
      ...
      if (distance > room.negotiationDistance) {
        socket.emit('error', `اللاعب بعيد جداً (${distance.toFixed(1)}م)`);
        return;
      }
      ...
      console.log(`📢 Negotiation: ${requestingPlayer.name} → ${targetPlayer.name} (${distance.toFixed(1)}m)`);
    });
    ...
    socket.on('eliminate-player', ({ roomId, eliminatedPlayerId }) => {
      ...
      if (eliminatedPlayer) {
        ...
        eliminatedPlayer.nearbyPlayers = []; // تنظيف القائمة
    ...
    // Voice chat proximity (للمحادثة الصوتية القريبة)
    socket.on('proximity-voice', ({ roomId, targetId, distance }) => {
      const room = rooms.get(roomId);
      if (!room || distance > 3) return;

      // تمرير إشارة القرب للمحادثة الصوتية
      io.to(targetId).emit('proximity-voice-signal', {
        fromId: socket.id,
        distance: distance,
        canTalk: distance <= 3
      });
    ...
    socket.on('disconnect', () => {
      ...
      if (roomId) {
        const room = rooms.get(roomId);
        if (room) {
          // إزالة اللاعب المنقطع من قوائم القرب للآخرين
          room.players.forEach(player => {
            if (player.nearbyPlayers) {
              player.nearbyPlayers = player.nearbyPlayers.filter(id => id !== socket.id);
            }
          });
          ...
          if (room.players.length === 0) {
            ...
            // إذا كان اللاعب المنقطع هو القائد، اختر قائد جديد
            if (room.currentLeader === socket.id) {
              const chassisPlayer = room.players.find(p => p.part === 'chassis' && p.isAlive);
              if (chassisPlayer) {
                room.currentLeader = chassisPlayer.id;
              }
            }
  ...
  server.listen(port, () => {
    ...
    // console.log('> 3D Game Mode with Proximity System Enabled 🎮👥`);
  });
});